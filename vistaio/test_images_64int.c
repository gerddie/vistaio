...
#define VTYPE int64_t
#define TYPEREPN VistaIOLong64Repn
...
int main(int UNUSED(argc), const char **UNUSED(args))
{
	...
	/* Create the image */ 
	image1 = VistaIOCreateImage(nz, ny, nx, VistaIOLong64Repn);

	
	/* put in some data */
	VistaIOLong64 *s = VistaIOPixelPtr(image1, 0, 0, 0);
	for (z = 0; z < nz; z++) 
		for (y = 0; y < ny; y++) 
			for (x = 0; x < nx; x++){
				*s++ = x + 10 * y + 0x8000000 * z;
			}
	...
	if ((file = fopen("test-image-64bit.v","w"))==0)
		VistaIOError("Unable to open file 'test-image-64bit.v' for writing");
...
#define CHECK_EQUAL_IMAGE_ATTRIBUTE(A, B, ATTR)				\
	...
	/* put in some data */
	VistaIOLong64 *t = VistaIOPixelPtr(image2, 0, 0, 0);
	for (z = 0; z < nz; z++) 
		for (y = 0; y < ny; y++) 
			for (x = 0; x < nx; x++, ++t){
				++tests_run;
				if (*t != x + 10 * y + 0x8000000 *  z) {
					VistaIOWarning("Pixel value at (%d, %d, %d): got %ld, expect %ld",
						 x,y,z,*t, x + 10 * y + 0x8000000 * z);
					++tests_failed; 
				}
			}
